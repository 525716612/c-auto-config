...
// ---------- 辅助函数 ----------
export function findRelativeFolder(rootPath: string, targetName: string, maxDepth: number): string | null {
    ...
    while (queue.length > 0) {
        ...
        if (baseName === targetName) {return relPath;}
        ...
        try {
            ...
            for (const entry of entries) {
                if (entry.isDirectory()) {
                    ...
                    if (entry.name === targetName) {return subRelPath;}
...
export function getSourceRelativePaths(
    ...
    if (!fs.existsSync(compileDbPath)) {return [];}
    try {
        ...
        for (const entry of entries) {
            ...
            if (!dir) {dir = rootPath;}
            const absoluteDir = path.isAbsolute(dir) ? dir : path.resolve(rootPath, dir);
            const absoluteFile = path.isAbsolute(file) ? file : path.resolve(absoluteDir, file);
            let relative = path.relative(rootPath, absoluteFile).replace(/\\/g, '/');
            paths.push(relative);
            if (outputChannel) {
                outputChannel.appendLine(`[调试] 转换: ${file} (dir: ${dir}) -> ${relative}`);
            }
        }
        return paths;
    } catch (err) {
...
export function getCygwinIncludePaths(cygwinBashPath: string): string[] {
    ...
    const possiblePaths = [
        ...
        path.join(cygwinRoot, 'opt', 'aeon', 'lib', 'gcc', 'aeon', '4.1.2', 'include')
    ];
...
export async function updateClangdExclude(
    ...
    const config = vscode.workspace.getConfiguration('cAutoConfig', vscode.Uri.file(rootPath));
    let templateObj = config.get<any>('clangdTemplate');
    if (!templateObj || typeof templateObj !== 'object') {
        templateObj = {};
        if (outputChannel) outputChannel.appendLine('[警告] 未找到有效的 clangdTemplate 配置，使用空对象');
    }
    ...
    const fixedLines = lines.map(line => /^Remove:/i.test(line) ? '  ' + line : line);
    ...
    if (cygwinBashPath) {
        ...
        if (includePaths.length > 0 && outputChannel) {
            outputChannel.appendLine(`[调试] 检测到 Cygwin 头文件路径: ${includePaths.join(', ')}`);
        }
        // 生成每一行，末尾不添加换行符（使用 join）
		extraIFlags = includePaths.map(p => `  - '-I${p.replace(/\\/g, '/')}'`).join('\n');
    ...
    if (extraIFlags) {
        ...
        if (addLineRegex.test(baseYaml)) {
            ...
            if (outputChannel) outputChannel.appendLine(`[调试] 已插入 ${addedCount} 个 -I 标志到 .clangd`);
        } else {
            // 如果没有 Add: 行，则创建 CompileFlags 结构
            baseYaml += `\nCompileFlags:\n  Add:\n${extraIFlags}\n`;
            if (outputChannel) outputChannel.appendLine('[调试] 未找到 Add: 行，已创建并插入 -I 标志');
        }
    } else {
        if (outputChannel) outputChannel.appendLine('[调试] 未配置 cygwinPath 或路径无效，不添加 -I 标志');
    }

    // 添加排除块
    let excludeBlock = '';
    if (excludePaths.length > 0) {
        const listItems = excludePaths.map(p => `    - ${p.replace(/\\/g, '/')}/.*`).join('\n');
        excludeBlock = `If:\n  PathExclude:\n${listItems}`;
    } else {
        excludeBlock = '# 无排除规则';
    }
...
export async function updateSettingsExclude(
    ...
    if (outputChannel) {
        ...
        outputChannel.appendLine(`  C_Cpp.files.exclude: 添加了 ${excludePaths.length} 条 glob 规则`);
        outputChannel.appendLine(`  search.exclude: 添加了 ${excludePaths.length} 条 glob 规则`);
